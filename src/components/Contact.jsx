...
${formData.message}`;
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-cyan-500/50 transition-all";
    ...
    <section id="contact" className="py-28 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-5 gap-12">
          {/* Left - Contact Info */}
          <motion.div initial={{ opacity: 0, x: -50 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="lg:col-span-2 space-y-8">
            <div className="space-y-6">
              {contactInfo.map((item, idx) => (
                <motion.a key={item.label} href={item.href} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.3 + idx * 0.1 }} whileHover={{ x: 5 }} className="flex items-center gap-4 group">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 group-hover:text-cyan-400 transition-colors">{item.value}</p>
                  </div>
                </motion.a>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.6 }}>
              <p className="text-white/40 text-sm uppercase tracking-wider mb-4">Follow Me</p>
              <div className="flex gap-3">
                {socials.map((social) => (
                  <motion.a key={social.label} href={social.href} aria-label={social.label} whileHover={{ scale: 1.1, y: -3 }} whileTap={{ scale: 0.95 }} className="w-11 h-11 rounded-xl glass flex items-center justify-center text-white/60 hover:text-cyan-400 hover:border-cyan-500/50 transition-all text-lg">{social.icon}</motion.a>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.7 }} className="glass-card p-6 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <p className="text-white/80 text-lg font-semibold mb-2">Let's work together!</p>
                <p className="text-white/50 text-sm">I'm currently available for freelance projects and full-time positions.</p>
              </div>
            </motion.div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 }} className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="glass-card p-8 space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="contact-name" className="block text-white/50 text-sm mb-2">Your Name</label>
                ...
                <div>
                  <label htmlFor="contact-email" className="block text-white/50 text-sm mb-2">Your Email</label>
              ...
              <div>
                <label htmlFor="contact-subject" className="block text-white/50 text-sm mb-2">Subject</label>
              ...
              <div>
                <label htmlFor="contact-message" className="block text-white/50 text-sm mb-2">Message</label>
              ...
              <motion.button type="submit" disabled={status === 'sending'} whileHover={{ scale: 1.02, boxShadow: "0 0 30px rgba(6,182,212,0.3)" }} whileTap={{ scale: 0.98 }} className="w-full py-4 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-semibold flex items-center justify-center gap-2 disabled:opacity-60">
